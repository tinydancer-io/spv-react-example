...
export async function monitorAndVerifyUpdates(
  ...
  // client.on("data", async function (d: any) {
    let received_update: Update = borsh.deserialize(UpdateSchema, bs58.decode(update)) as any;
 console.dir("received_update",received_update)
    ...
    for (const p of bankhash_proof.proofs) {
      let response = await verifyLeavesAgainstBankhash(
        p,
        bankhash,
