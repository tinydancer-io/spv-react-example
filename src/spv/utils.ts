...
export async function hashSolanaAccount(
  ...
  // const hasher = await createBLAKE3(128);
  ...
  hasher.update(
	...
	// console.dir("invalid ",Buffer.from(data))
  ...
  hasher.update(Buffer.from(owner));
  hasher.update(Buffer.from(pubkey));
...
export async function verifyLeavesAgainstBankhash(
  ...
  if (!Buffer.from(data.hash).equals(Buffer.from(computedAccountHash))) {
    console.warn(`account data does not match account hash, left: ${bs58.encode(data.hash)}\nright: ${bs58.encode(computedAccountHash)}`);
		return false;
  }
  ...
  if (!Buffer.from(bankhash).equals(Buffer.from(computedBankhash))) {
     console.warn("bank hash does not match data");
		 return false;
  }

  if (!verifyProof(data.hash, proof, accountDeltaRoot)) {
    console.warn("account merkle proof verification failure");
		return false;
  }
	return true
}
